...
function doAlter($confirm) {
	...
	if ($confirm) {
		...
		htmlspecialchars_nc($_REQUEST['alterdatabase']), "\" /></td></tr>\n";
...
function doCreate($msg = '') {
	...
	if ($data->hasDatabaseCollation()) {
		...
		echo "\t\t<td class=\"data1\">\n";
		echo "\t\t\t<input name=\"formCollate\" value=\"", htmlspecialchars_nc($_POST['formCollate']), "\" />\n";
		echo "\t\t</td>\n\t</tr>\n";

		// LC_CTYPE
		echo "\t<tr>\n\t\t<th class=\"data left\">{$lang['strctype']}</th>\n";
		echo "\t\t<td class=\"data1\">\n";
		echo "\t\t\t<input name=\"formCType\" value=\"", htmlspecialchars_nc($_POST['formCType']), "\" />\n";
		echo "\t\t</td>\n\t</tr>\n";
	}
