...
function doAccount($msg = '')
{
	...
	$userdata = $roleActions->getUser($server_info['username']);
	...
	if ($userdata->recordCount() > 0) {
		$userdata->fields['usesuper'] = $pg->phpBool($userdata->fields['usesuper']);
		$userdata->fields['usecreatedb'] = $pg->phpBool($userdata->fields['usecreatedb']);
		echo "<table>\n";
...
function doChangePassword($confirm, $msg = '')
{
	...
	if ($confirm) {
		...
		echo "\t\t<td><input type=\"password\" name=\"password\" size=\"32\" value=\"",
			htmlspecialchars_nc($_POST['password']), "\" /></td>\n\t</tr>\n";
		...
		elseif ($_POST['password'] != $_POST['confirm'])
			...
			$status = $roleActions->changePassword(
...
function doEdit($msg = '')
{
	...
	$userdata = $roleActions->getUser($_REQUEST['username']);

	if ($userdata->recordCount() > 0) {
		$server_info = $misc->getServerInfo();
		$canRename = $pg->hasUserRename() && ($_REQUEST['username'] != $server_info['username']);
		$userdata->fields['usesuper'] = $pg->phpBool($userdata->fields['usesuper']);
		$userdata->fields['usecreatedb'] = $pg->phpBool($userdata->fields['usecreatedb']);
		...
		echo "\t\t<td class=\"data1\">", ($canRename ? "<input name=\"newname\" size=\"15\" maxlength=\"{$pg->_maxNameLen}\" value=\"" . htmlspecialchars_nc($_POST['newname']) . "\" />" : $misc->printVal($userdata->fields['usename'])), "</td>\n\t</tr>\n";
		...
		echo "\t\t<td class=\"data1\"><input size=\"16\" name=\"formExpires\" value=\"", htmlspecialchars_nc($_POST['formExpires']), "\" /></td>\n\t</tr>\n";
		echo "\t<tr>\n\t\t<th class=\"data left\">{$lang['strpassword']}</th>\n";
		echo "\t\t<td class=\"data1\"><input type=\"password\" size=\"16\" name=\"formPassword\" value=\"", htmlspecialchars_nc($_POST['formPassword']), "\" /></td>\n\t</tr>\n";
		echo "\t<tr>\n\t\t<th class=\"data left\">{$lang['strconfirm']}</th>\n";
		echo "\t\t<td class=\"data1\"><input type=\"password\" size=\"16\" name=\"formConfirm\" value=\"\" /></td>\n\t</tr>\n";
		echo "</table>\n";
		echo "<p><input type=\"hidden\" name=\"action\" value=\"save_edit\" />\n";
		echo "<input type=\"hidden\" name=\"username\" value=\"", htmlspecialchars_nc($_REQUEST['username']), "\" />\n";
		echo $misc->form;
...
function doSaveEdit()
{
	...
	if (isset($_POST['newname']) && $_POST['newname'] == '')
		...
		if (isset($_POST['newname'])) {
			$status = $roleActions->setRenameUser(
				$_POST['username'],
				$_POST['formPassword'],
...
function doDrop($confirm)
{
	...
	if ($confirm) {
		...
		$status = $roleActions->dropUser($_REQUEST['username']);
		if ($status == 0)
...
function doCreate($msg = '')
{
	...
	echo "\t\t<td class=\"data1\"><input size=\"15\" maxlength=\"{$pg->_maxNameLen}\" name=\"formUsername\" value=\"", htmlspecialchars_nc($_POST['formUsername']), "\" /></td>\n\t</tr>\n";
	echo "\t<tr>\n\t\t<th class=\"data left\">{$lang['strpassword']}</th>\n";
	echo "\t\t<td class=\"data1\"><input size=\"15\" type=\"password\" name=\"formPassword\" value=\"", htmlspecialchars_nc($_POST['formPassword']), "\" /></td>\n\t</tr>\n";
	echo "\t<tr>\n\t\t<th class=\"data left\">{$lang['strconfirm']}</th>\n";
	echo "\t\t<td class=\"data1\"><input size=\"15\" type=\"password\" name=\"formConfirm\" value=\"", htmlspecialchars_nc($_POST['formConfirm']), "\" /></td>\n\t</tr>\n";
	echo "\t<tr>\n\t\t<th class=\"data left\"><label for=\"formSuper\">{$lang['strsuper']}</label></th>\n";
	echo "\t\t<td class=\"data1\"><input type=\"checkbox\" id=\"formSuper\" name=\"formSuper\"", (isset($_POST['formSuper'])) ? ' checked="checked"' : '', " /></td>\n\t</tr>\n";
	echo "\t<tr>\n\t\t<th class=\"data left\"><label for=\"formCreateDB\">{$lang['strcreatedb']}</label></th>\n";
	echo "\t\t<td class=\"data1\"><input type=\"checkbox\" id=\"formCreateDB\" name=\"formCreateDB\"", (isset($_POST['formCreateDB'])) ? ' checked="checked"' : '', " /></td>\n\t</tr>\n";
	echo "\t<tr>\n\t\t<th class=\"data left\">{$lang['strexpires']}</th>\n";
	echo "\t\t<td class=\"data1\"><input size=\"30\" name=\"formExpires\" value=\"", htmlspecialchars_nc($_POST['formExpires']), "\" /></td>\n\t</tr>\n";
	echo "</table>\n";
